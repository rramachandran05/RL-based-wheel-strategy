...
3. **Opening recommendations** per universe ticker from the latest row: Q-state → B3 action → chain (real prev-close snapshot where accrued, else synthetic with G1-fitted `iv_uplift`) → SPEC-011 MCB gates (layer-A mask, net-basis ceiling pre-filter) → selector → book-level risk engine (SPEC-004 §2.1, fed by `BookState` from the synced positions) → recommendation object (SPEC-006 §6 shape).
   - **Book-aware expiry selection (2026-09-11):** the selector receives `BookState.expiry_week_headroom(cap, NAV)` and drops put candidates whose escrow would breach the RISK-5 week cap, so it never walks into an already-capped week when a sibling expiry in the 25–45 DTE window would pass. RISK-5 in the engine remains the hard backstop.
   - **Contract columns always populated (2026-09-11):** `Strike | DTE | Δ | Model prem` are shown for every row. For SELL_PUT they are the recommendation; for WAIT rows (marked `WAIT†`) they show the contract the selector *would* have chosen — the risk-blocked or MCB-blocked candidate, or the CONSERVATIVE-tier reference when the policy itself chose WAIT — carried in the JSON as `candidate_contract`, never as `contract`, so the decision record is unchanged. A footnote under the table states this.
   - **No Candidates section** (SPEC-010 Feed B retired 2026-09-11).
   - **Openings columns (2026-09-11, revised same day):** `Ticker | Price | State | Decision | Reason | Strike | DTE | Δ | Model prem | Posture | Ceiling | Prem req | Ref high (date) | Typical historical pullback | Larger historical pullback | MCB flags`. **Ceiling = the MCB value** (`wheel_entry`). **Decision** is one status per row with one reason (`decision` in the JSON, `decision_status()`): `Candidate`; `Candidate — review required`; `Blocked — position limit` (risk engine, with the binding ratio vs cap, e.g. "exposure to MSFT would be 24.4% of NAV vs the 15% cap"; also the all-weeks-capped case); `Blocked — MCB ceiling`; `Blocked — layer A`; `Reference only` (policy WAIT; CONSERVATIVE-tier reference shown); `No contract`. The ambiguous `SELL_PUT ⚠ REVIEW` / `WAIT†` markers are retired. **Pullbacks** translate mcb-wheel's `dd50`/`dd75` into a resulting price and dollar decline from the report's trailing high `ref_high`, with the date our close series printed that high (`164.00 (−36.00, 18.0%)`); when no trailing high is supplied the percentage is applied to today's price and the cell is marked *illustrative*. The Legend carries a column-definitions table for both the openings and covered-call tables.
   - **Capital and limits section (2026-09-11):** rendered before the openings from `capital_summary()` (`capital` in the JSON): total account value (NAV, labelled *assumed via `--cash`*), cash reserved for open puts (escrow), stock sleeve (positions-tab shares × last close), cash available for new trades — **Estimated** (NAV − escrow − stock sleeve) unless the broker figure is passed via `--available-cash`, then **Verified** — new escrow proposed in this brief and the remainder if every candidate executes. A negative estimate renders an *Inconsistent inputs* row (escrow + stock exceed the assumed NAV: account larger than `--cash`, or puts not fully cash-secured). Below it, an **Expiration-week capacity** table by calendar dates: existing commitment, remaining capacity, proposed this brief (tickers), if-all-executed (% NAV), status (within cap / over by $x).
   - **Cumulative review (2026-09-11, SPEC-004 §2.10):** after all rows are formed, the joint RISK-5/RISK-8 effect of executing every SELL_PUT is recomputed and surfaced as `⚠ REVIEW (cumulative)` — on the affected rows and in the header — with room left and an example subset that fits. Never a block; the user picks the subset.
3b. **Covered-call recommendations for every universe name** (2026-09-11, SPEC-011 §2 rule 7): stock-side policy tier → capped by mcb-wheel `cc_posture` (+ Bull-Trend upside-protection override) → call chain → selector (cost-basis floor where a basis exists) → **CCA classification** (`strike + premium` vs `selected_call_away_level`: ASSIGNMENT_ACCEPTABLE / INCOME_WAIT with shortfall → ⚠ REVIEW, never a rejection). Rendered as two tables (2026-09-11 revision): **Holdings — shares available to cover** (`Ticker | Price | State | Decision | Reason | Strike | DTE | Δ | Model prem | CC posture | CCA | Exit check | Shares held | Committed to open calls | Additional contracts coverable`) and a **Hypothetical watchlist — no shares held** (same without the share columns, every row `Reference only`). Shares held come from the MCB positions tab; committed = 100 × open CC contracts in `positions.csv`; coverable = `(held − committed) ÷ 100`. Statuses (`call_decision_status()`): `Candidate`; `Candidate — review required` (INCOME_WAIT); `Blocked — no uncovered shares` (coverable = 0, e.g. MSFT 400 held / 400 committed); `Blocked — cost basis` (price below basis); `No contract` (thin chain); `Wait — policy`; `Reference only`. The risk engine (RISK-2) runs where shares exist. Carried as `covered_calls` in the JSON; not written to the trajectory log.
